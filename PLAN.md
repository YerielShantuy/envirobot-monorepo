...
- [ ] **Email organisers** (createunsw@gmail.com AND unsw@studentenergy.org):
  ...
  2. Ask whether full RC driving qualifies for the 18–22 band of Cat 1 ("minor navigation assistance") when both sensors are accurate
  3. *(optional since 2026-07-16)* WiFi webapp approval (rulebook §4.6) — `arduino/tools/live_plot.py` (matplotlib live table + labelled charts + CSV) is now the judged output and is already an approved format; webapp is showcase-only
...
- [x] **Voltage divider** on SEN0189 output → **GPIO34** (user-confirmed 2026-07-16; this line previously said GPIO36 — config.h pin 34 is correct). Firmware assumes ratio **2.0** (equal resistors); if different values were used, update `TURBIDITY_DIVIDER_RATIO` in `config.h` before calibrating
- [ ] **Rewire for revised BOM** (2026-07-10 component change — MPU6050 + VL53L0X dropped, HC-SR04 added): motors 13/14/4 + 18/19/23 · servo 26 · I2C SDA 21 / SCL 22 (TCS34725 only) · HC-SR04 TRIG 16 / ECHO 17 (ECHO through 5V→3.3V divider) · turbidity 34 · soil 35
...
- [ ] **Aesthetics (Cat 5, 5 pts, cheapest points in the rulebook)**: team name label/plate, cable dress (sleeves/ties), tidy component layout, consistent finish — one evening of work
...
7. [ ] Full 8-min RC rehearsal: 12 samples with sector tagging, `live_plot.py --url http://192.168.4.1/samples` running on the laptop throughout → End Run → `GET /data` → drop JSON into `/viz` — the complete judge-facing flow (run clock starts on first drive/sample; watch the ⏱ readout, amber at 7:00)

Served pages: `arduino/rc_page/index.html` (RC, at `/`) and `arduino/test_page/index.html`
(diagnostics, at `/test`). After editing either, regenerate both PROGMEM headers:
```powershell
node arduino/tools/gen-pages.cjs
```
Emits `envirobot/rc_page.h` + `envirobot/test_page.h`. Adding a page = one row in the
`PAGES` table in that script.

**Partition scheme**: the default 1.3MB app partition is at 94% with both pages compiled in.
Set Tools → Partition Scheme → **"No OTA (2MB APP/2MB SPIFFS)"** (`PartitionScheme=no_ota`)
— same firmware drops to 59% and SPIFFS grows to 2MB. Changing the scheme erases SPIFFS on
the next flash, so do it before a run, not between a run and its `/data` download.
...
- [ ] Arena test: wander + wall bounce (HC-SR04), pond stop → sample → backoff, soil detect
...
- [ ] 3-min innovation pitch for Testing Day robot assessment: double-arm single-servo story · median-of-60 sampling window (rulebook's own "multi-sample averaging" example) · WiFi dead-man failsafe + armed-start gate · quadrant sector estimation + REZERO recovery · iteration evidence (test sketches, audit trail, component revisions)
...
- [x] **Deploy** — live since 2026-07-09 at `https://envirobot.yerielph.workers.dev` (verified 2026-07-16: root 200, `/api/viz-runs` 200 with a test run). Firmware `UPLOAD_URL` now points at it
- [ ] Re-seed `/code`: `node scripts/sync-code.mjs --push` (now redacts `WIFI_STA_*` creds — live copy is an old pre-STA version; push after firmware settles)
- [ ] Clear any stale `docs_pages` rows in Supabase (they override the baked-in docs)
...
- [ ] Run clock: `t_s`/`total_duration_s` start at first drive/sample (RC) or START (auto), not boot; RC page ⏱ counts up and freezes on End Run
- [ ] `live_plot.py` rehearsed both modes: `--url` over RobotAP during a run, `--port` on USB for inspection; `samples.csv` produced
